...
class PreprocessedDataset(Dataset):
    ...
    def _process(
        ...
        return (ti1.squeeze(), vae_latent.squeeze(), mask.squeeze()) if not self.is_sdxl else ((ti1.squeeze(), ti2.squeeze()), vae_latent.squeeze(), mask.squeeze())
...
def load_models(pretrained_model_name_or_path, revision, device, weight_dtype, is_sdxl=True):
    ...
    return (
        tokenizer_one,
        tokenizer_two if is_sdxl else None,
        noise_scheduler,
        text_encoder_one,
        text_encoder_two if is_sdxl else None,
        vae,
        unet,
    )
